...
(defn attr
  ...
  {:added "2.4"} [name value]
  (Attribute. name value))

(defn attr-value
  "adjusts the attribute value for input"
  {:added "2.4"} [k v]
  (cond (= k "owner") (lookup-owner v)
        (= k "group") (lookup-group v)
        (= k "fileKey") (throw (Exception. "Cannot make fileKey"))
        (= k "permissions") (PosixFilePermissions/fromString v)
...
(defn attributes
  ...
  {:added "2.4"} [path]
  (-> (path/path path)
      (Files/readAttributes ^String (case common/*system*
                              :unix    "unix:*"
                              :windows "dos:*")
                            common/*no-follow*)
      (attrs->map)))
...
(defn set-attributes
  ...
  (reduce-kv (fn [_ k v]
               (-> (path/path path)
                   (Files/setAttribute (str (case common/*system*
                                              :unix    "unix:"
                                              :windows "dos:")
